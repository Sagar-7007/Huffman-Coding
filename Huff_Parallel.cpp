...
void workerFunction(const string& text, int threadId, int numThreads, vector<char>& compressedData) {
...
int main(int argc, char* argv[]) {
    ...
    for (int it = 0; it < numIterations; ++it) {
        ...
        for (int i = 0; i < numThreads; ++i) {
            threads.emplace_back(workerFunction, text, i, numThreads, ref(compressedParts[i]));
        }
